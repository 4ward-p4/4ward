...
class GoldenErrorTest(private val testName: String) {
  ...
  private fun triggerRegisterIndexOutOfBounds() {
    ...
    val entity = FourwardTestHarness.buildRegisterEntry(registerId, index = 999, value = 0)
    harness.modifyEntry(entity)
  }
  ...
  private fun triggerCounterIndexOutOfBounds() {
    ...
    val entity = FourwardTestHarness.buildCounterEntry(counterId, index = 999)
    harness.modifyEntry(entity)
  }
  ...
  private fun triggerMeterIndexOutOfBounds() {
    ...
    val entity = FourwardTestHarness.buildMeterEntry(meterId, index = 999)
    harness.modifyEntry(entity)
  }
  ...
  private fun triggerActionNotValidForProfile() {
    ...
    val entity =
      FourwardTestHarness.buildMemberEntity(
  ...
  private fun triggerActionProfileAtCapacity() {
    ...
    harness.installEntry(FourwardTestHarness.buildMemberEntity(profile.preamble.id, 1, actionId))
    harness.installEntry(FourwardTestHarness.buildMemberEntity(profile.preamble.id, 2, actionId))
    harness.installEntry(FourwardTestHarness.buildMemberEntity(profile.preamble.id, 3, actionId))
  }
  ...
  private fun triggerActionProfileGroupMaxSize() {
    ...
    harness.installEntry(FourwardTestHarness.buildGroupEntity(profile.preamble.id, 1, listOf(1, 2)))
  }
  ...
  private fun triggerUnknownProfileIdMember() {
    harness.loadPipeline(loadBasicTable())
    harness.installEntry(FourwardTestHarness.buildMemberEntity(99999, 1, 1))
  }

  @Suppress("MagicNumber")
  private fun triggerUnknownProfileIdGroup() {
    harness.loadPipeline(loadBasicTable())
    harness.installEntry(FourwardTestHarness.buildGroupEntity(99999, 1, listOf(1)))
  }
  ...
  private fun triggerUnknownActionInProfile() {
    ...
    harness.loadPipeline(config)
    val profile = config.p4Info.actionProfilesList.first()
    harness.installEntry(FourwardTestHarness.buildMemberEntity(profile.preamble.id, 1, 99999))
  }
  ...
  private fun triggerConstraintValidationFailed() {
    ...
    harness.close()
    FourwardTestHarness(constraintValidatorBinary = VALIDATOR_BINARY).use { constrainedHarness ->
  ...
  companion object {
    ...
    private fun goldenDir(): java.nio.file.Path =
      fourward.bazel.repoRoot.resolve("grpc/golden_errors")
  }
}
